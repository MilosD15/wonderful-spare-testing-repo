...
if (document.querySelector("[data-loyalty-rewards-section]")) {
  ...
  function handleLottieAnimation() {
    if (isSectionInViewPort(loggedInHeroSection) && sectionWasAlreadyInViewport === false) {
      sectionWasAlreadyInViewport = true;
      reloadPlayer(loyaltyRewardsLottiePlayer);
    }
    if (!isSectionInViewPort(loggedInHeroSection)) {

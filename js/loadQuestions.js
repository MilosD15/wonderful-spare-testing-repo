...
const randomQuestions = await getQuestions();
...
function renderQuestion(questionObject) {
  ...
  answerText.textContent = questionObject.answer;
...
function composeQuestionObjects(randomQuestions, questionThemesObjects) {
  const questionObjects = randomQuestions.map((question, index) => {
    const questionTheme = questionThemesObjects[index];
...
async function getQuestions(count = 5) {
  ...
  const numberOfQuestions = count > allQuestions.length ? allQuestions.length : count;
  const questions = allQuestions.slice(0, numberOfQuestions);
